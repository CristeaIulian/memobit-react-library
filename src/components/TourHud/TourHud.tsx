...
export interface TourHudProps {
    ...
    onNext: () => void;
    onExit: () => void;
    /** Fixed HUD width. Numbers are treated as px. */
    width?: number | string;
    className?: string;
}
...
export const TourHud: React.FC<TourHudProps> = ({
    ...
    const widthStyle =
        width === undefined
            ? undefined
            : ({
                  '--tour-hud-width': typeof width === 'number' ? `${width}px` : width,
              } as React.CSSProperties);
    const classNames = ['tour-hud', width !== undefined && 'tour-hud--fixed-width', className].filter(Boolean).join(' ');
    const contextParts = [label, `Stop ${index + 1} / ${total}`, stepLabel].filter(Boolean);

    return (
        <div className={classNames} style={widthStyle}>
            <div className="tour-hud__body">
                <div className="tour-hud__context">{contextParts.join(' \u00b7 ')}</div>
