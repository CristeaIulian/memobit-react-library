...
export const PEDIATRIC_THRESHOLDS: Record<'male' | 'female', Record<number, [number, number, number]>> = {
    male: {
        2: [14.7, 17.4, 18.4],
        3: [14.0, 16.7, 17.9],
        4: [13.7, 16.3, 17.6],
...
export const calculateBMI = (weight: number, height: number): number => weight / (height / 100) ** 2;

export const getIndicatorPosition = (bmi: number, ranges: BMIRange[]): number => {
...
export const getIndicatorColor = (bmi: number, ranges: BMIRange[]): string => {
    for (const range of ranges) {
...
export const getRangeLabel = (range: BMIRange, index: number, total: number): string => {
